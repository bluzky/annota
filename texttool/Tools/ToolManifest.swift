...
/// type it produces.
struct ToolManifest<Obj: CopyableCanvasObject> {
    let tool: any CanvasTool
    let discriminator: String
    let interactiveView: @MainActor (Obj, Bool, CanvasViewModel) -> AnyView
    let exportView: @MainActor (Obj) -> AnyView
}
...
struct ObjectManifest<Obj: CopyableCanvasObject> {
    let discriminator: String
    let interactiveView: @MainActor (Obj, Bool, CanvasViewModel) -> AnyView
    let exportView: @MainActor (Obj) -> AnyView
}
